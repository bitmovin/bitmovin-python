...
from bitmovin import Bitmovin, ACLEntry, ACLPermission, EncodingOutput, Period, VideoAdaptationSet, \
    ...
    Stream, StreamInput, MuxingStream, FMP4Muxing, MarlinDRM, AbstractFMP4Representation, HlsManifest, VideoMedia
from tests.bitmovin import BitmovinTestCase


class VideoMediaTests(BitmovinTestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_add_video_media(self):
        ...
        sample_video_media = self._get_sample_video_media()
        video_media_resource_response = self.bitmovin.manifests.HLS.VideoMedia.create(
            object_=sample_video_media, manifest_id=manifest_resource_response.resource.id)

        self.assertIsNotNone(video_media_resource_response)
        self.assertIsNotNone(video_media_resource_response.resource)
        self.assertIsNotNone(video_media_resource_response.resource.id)
        self._compare_video_medias(sample_video_media, video_media_resource_response.resource)

    def test_list_video_media(self):
        sample_manifest = self._get_sample_manifest()
        manifest_resource_response = self.bitmovin.manifests.HLS.create(sample_manifest)
        self.assertIsNotNone(manifest_resource_response)
        self.assertIsNotNone(manifest_resource_response.resource)
        ...
        manifests_resource_response = self.bitmovin.manifests.HLS.VideoMedia.list(manifest_id=manifest_resource_response.resource.id, limit=1)
    ...
    def test_retrieve_video_media(self):
        sample_manifest = self._get_sample_manifest()
        manifest_resource_response = self.bitmovin.manifests.HLS.create(sample_manifest)
        self.assertIsNotNone(manifest_resource_response)
        self.assertIsNotNone(manifest_resource_response.resource)
        ...
        retrieve_manifest_resource_response = self.bitmovin.manifests.HLS.VideoMedia.retrieve(
            manifest_id=manifest_resource_response.resource.id, media_id=video_media_resource_response.resource.id)

        self.assertIsNotNone(retrieve_manifest_resource_response)
        self.assertTrue(isinstance(retrieve_manifest_resource_response.resource, VideoMedia))
        self._compare_video_medias(sample_video_media, retrieve_manifest_resource_response.resource)
    ...
    def test_delete_video_media(self):
        ...
        delete_sample_video_media_resource_response = self.bitmovin.manifests.HLS.VideoMedia.delete(
            manifest_id=manifest_resource_response.resource.id, media_id=video_media_resource_response.resource.id)

        self.assertIsNotNone(delete_sample_video_media_resource_response)
        self.assertIsNotNone(delete_sample_video_media_resource_response.resource)
        self.assertIsNotNone(delete_sample_video_media_resource_response.resource.id)
        self.assertEqual(video_media_resource_response.resource.id,
                         delete_sample_video_media_resource_response.resource.id)
    ...
    def _compare_video_medias(self, first: VideoMedia, second: VideoMedia):
        self.assertEqual(first.name, second.name)
    ...
    def _get_sample_video_media(self):
        encoding_id = self.sampleEncoding.id
        stream_id = self.sampleStream.id
        muxing_id = self.sampleMuxing.id
        drm_id = self.sampleDrm.id

        video_media = VideoMedia(group_id='video_group_1', language='en', assoc_language='en', name='Video Media Test',
