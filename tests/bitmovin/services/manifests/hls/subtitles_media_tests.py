...
from bitmovin import Bitmovin, ACLEntry, ACLPermission, EncodingOutput, Period, VideoAdaptationSet, \
    ...
    Stream, StreamInput, MuxingStream, FMP4Muxing, MarlinDRM, AbstractFMP4Representation, HlsManifest, SubtitlesMedia
from tests.bitmovin import BitmovinTestCase


class SubtitlesMediaTests(BitmovinTestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_add_subtitles_media(self):
        ...
        sample_subtitles_media = self._get_sample_subtitles_media()
        subtitles_media_resource_response = self.bitmovin.manifests.HLS.SubtitlesMedia.create(
            object_=sample_subtitles_media, manifest_id=manifest_resource_response.resource.id)

        self.assertIsNotNone(subtitles_media_resource_response)
        self.assertIsNotNone(subtitles_media_resource_response.resource)
        self.assertIsNotNone(subtitles_media_resource_response.resource.id)
        self._compare_subtitles_medias(sample_subtitles_media, subtitles_media_resource_response.resource)

    def test_list_subtitles_media(self):
        sample_manifest = self._get_sample_manifest()
        manifest_resource_response = self.bitmovin.manifests.HLS.create(sample_manifest)
        self.assertIsNotNone(manifest_resource_response)
        self.assertIsNotNone(manifest_resource_response.resource)
        ...
        manifests_resource_response = self.bitmovin.manifests.HLS.SubtitlesMedia.list(manifest_id=manifest_resource_response.resource.id, limit=1)
    ...
    def test_retrieve_subtitles_media(self):
        sample_manifest = self._get_sample_manifest()
        manifest_resource_response = self.bitmovin.manifests.HLS.create(sample_manifest)
        self.assertIsNotNone(manifest_resource_response)
        self.assertIsNotNone(manifest_resource_response.resource)
        ...
        retrieve_manifest_resource_response = self.bitmovin.manifests.HLS.SubtitlesMedia.retrieve(
            manifest_id=manifest_resource_response.resource.id, media_id=subtitles_media_resource_response.resource.id)

        self.assertIsNotNone(retrieve_manifest_resource_response)
        self.assertTrue(isinstance(retrieve_manifest_resource_response.resource, SubtitlesMedia))
        self._compare_subtitles_medias(sample_subtitles_media, retrieve_manifest_resource_response.resource)

    def test_delete_subtitles_media(self):
        sample_manifest = self._get_sample_manifest()
        manifest_resource_response = self.bitmovin.manifests.HLS.create(sample_manifest)
        self.assertIsNotNone(manifest_resource_response)
        self.assertIsNotNone(manifest_resource_response.resource)
        ...
        delete_sample_subtitles_media_resource_response = self.bitmovin.manifests.HLS.SubtitlesMedia.delete(
            manifest_id=manifest_resource_response.resource.id, media_id=subtitles_media_resource_response.resource.id)

        self.assertIsNotNone(delete_sample_subtitles_media_resource_response)
        self.assertIsNotNone(delete_sample_subtitles_media_resource_response.resource)
        self.assertIsNotNone(delete_sample_subtitles_media_resource_response.resource.id)
        self.assertEqual(subtitles_media_resource_response.resource.id,
                         delete_sample_subtitles_media_resource_response.resource.id)
    ...
    def _compare_subtitles_medias(self, first: SubtitlesMedia, second: SubtitlesMedia):
        self.assertEqual(first.name, second.name)
    ...
    def _get_sample_subtitles_media(self):
        encoding_id = self.sampleEncoding.id
        stream_id = self.sampleStream.id
        muxing_id = self.sampleMuxing.id
        drm_id = self.sampleDrm.id

        subtitles_media = SubtitlesMedia(group_id='subs_group_1', language='en', assoc_language='en', name='Subs Media Test',
