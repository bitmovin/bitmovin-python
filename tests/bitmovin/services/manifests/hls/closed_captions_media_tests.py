...
from bitmovin import Bitmovin, ACLEntry, ACLPermission, EncodingOutput, Encoding, \
    Stream, StreamInput, MuxingStream, FMP4Muxing, MarlinDRM, HlsManifest, ClosedCaptionsMedia
from tests.bitmovin import BitmovinTestCase


class ClosedCaptionsMediaTests(BitmovinTestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_add_closed_captions_media(self):
        ...
        sample_closed_captions_media = self._get_sample_closed_captions_media()
        closed_captions_media_resource_response = self.bitmovin.manifests.HLS.ClosedCaptionsMedia.create(
            object_=sample_closed_captions_media, manifest_id=manifest_resource_response.resource.id)

        self.assertIsNotNone(closed_captions_media_resource_response)
        self.assertIsNotNone(closed_captions_media_resource_response.resource)
        self.assertIsNotNone(closed_captions_media_resource_response.resource.id)
        self._compare_closed_captions_medias(sample_closed_captions_media, closed_captions_media_resource_response.resource)

    def test_list_closed_captions_media(self):
        sample_manifest = self._get_sample_manifest()
        manifest_resource_response = self.bitmovin.manifests.HLS.create(sample_manifest)
        self.assertIsNotNone(manifest_resource_response)
        self.assertIsNotNone(manifest_resource_response.resource)
        ...
        manifests_resource_response = self.bitmovin.manifests.HLS.ClosedCaptionsMedia.list(manifest_id=manifest_resource_response.resource.id, limit=1)
    ...
    def test_retrieve_closed_captions_media(self):
        sample_manifest = self._get_sample_manifest()
        manifest_resource_response = self.bitmovin.manifests.HLS.create(sample_manifest)
        self.assertIsNotNone(manifest_resource_response)
        self.assertIsNotNone(manifest_resource_response.resource)
        ...
        retrieve_manifest_resource_response = self.bitmovin.manifests.HLS.ClosedCaptionsMedia.retrieve(
            manifest_id=manifest_resource_response.resource.id, media_id=closed_captions_media_resource_response.resource.id)

        self.assertIsNotNone(retrieve_manifest_resource_response)
        self.assertTrue(isinstance(retrieve_manifest_resource_response.resource, ClosedCaptionsMedia))
        self._compare_closed_captions_medias(sample_closed_captions_media, retrieve_manifest_resource_response.resource)

    def test_delete_closed_captions_media(self):
        sample_manifest = self._get_sample_manifest()
        manifest_resource_response = self.bitmovin.manifests.HLS.create(sample_manifest)
        self.assertIsNotNone(manifest_resource_response)
        self.assertIsNotNone(manifest_resource_response.resource)
        ...
        delete_sample_closed_captions_media_resource_response = self.bitmovin.manifests.HLS.ClosedCaptionsMedia.delete(
            manifest_id=manifest_resource_response.resource.id, media_id=closed_captions_media_resource_response.resource.id)

        self.assertIsNotNone(delete_sample_closed_captions_media_resource_response)
        self.assertIsNotNone(delete_sample_closed_captions_media_resource_response.resource)
        self.assertIsNotNone(delete_sample_closed_captions_media_resource_response.resource.id)
        self.assertEqual(closed_captions_media_resource_response.resource.id,
                         delete_sample_closed_captions_media_resource_response.resource.id)
    ...
    def _compare_closed_captions_medias(self, first: ClosedCaptionsMedia, second: ClosedCaptionsMedia):
        self.assertEqual(first.name, second.name)
        ...
        self.assertEqual(first.instreamId, second.instreamId)
    ...
    def _get_sample_closed_captions_media(self):
        encoding_id = self.sampleEncoding.id
        stream_id = self.sampleStream.id
        muxing_id = self.sampleMuxing.id
        drm_id = self.sampleDrm.id

        closed_captions_media = ClosedCaptionsMedia(group_id='audio_group_1', language='en', assoc_language='en', name='Audio Media Test',
                                 ...
                                 start_segment_number=0, instream_id='deadbeef')

        return closed_captions_media
