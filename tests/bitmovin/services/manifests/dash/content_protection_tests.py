...
from bitmovin import Bitmovin, DashManifest, ACLEntry, ACLPermission, EncodingOutput, Period, VideoAdaptationSet, \
    ...
    Stream, StreamInput, MuxingStream, FMP4Muxing, MarlinDRM, AbstractFMP4Representation, ContentProtection
from tests.bitmovin import BitmovinTestCase


class ContentProtectionTests(BitmovinTestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def setUp(self):
        ...
        self.sampleStream = self._create_sample_stream()  # type: Stream
    ...
    def test_add_content_protection_to_adaptionset(self):
        ...
        content_protection_resource_response = self.bitmovin.manifests.DASH.add_content_protection_to_adaptionset(
            object_=sample_content_protection,
            manifest_id=manifest_resource_response.resource.id,
            period_id=period_resource_response.resource.id,
            adaptationset_id=adaptationset_resource_response.resource.id
        )
        self.assertIsNotNone(content_protection_resource_response)
        self.assertIsNotNone(content_protection_resource_response.resource)
        self.assertIsNotNone(content_protection_resource_response.resource.id)
        self._compare_content_protections(sample_content_protection, content_protection_resource_response.resource)

    def test_add_content_protection_to_fmp4_representation(self):
        sample_manifest = self._get_sample_manifest()
        manifest_resource_response = self.bitmovin.manifests.DASH.create(sample_manifest)
        self.assertIsNotNone(manifest_resource_response)
        self.assertIsNotNone(manifest_resource_response.resource)
        ...
        sample_content_protection = self._get_sample_content_protection()
        content_protection_resource_response = self.bitmovin.manifests.DASH.add_content_protection_to_fmp4_represenation(
            object_=sample_content_protection,
            manifest_id=manifest_resource_response.resource.id,
            period_id=period_resource_response.resource.id,
            adaptationset_id=adaptationset_resource_response.resource.id,
            representation_id=representation_resource_response.resource.id
        )
        self.assertIsNotNone(content_protection_resource_response)
        self.assertIsNotNone(content_protection_resource_response.resource)
        self.assertIsNotNone(content_protection_resource_response.resource.id)
    ...
    def test_add_content_protection_to_drm_fmp4_representation(self):
        sample_manifest = self._get_sample_manifest()
        manifest_resource_response = self.bitmovin.manifests.DASH.create(sample_manifest)
        self.assertIsNotNone(manifest_resource_response)
        self.assertIsNotNone(manifest_resource_response.resource)
        ...
        sample_content_protection = self._get_sample_content_protection()
        content_protection_resource_response = self.bitmovin.manifests.DASH.add_content_protection_to_drm_fmp4_represenation(
            object_=sample_content_protection,
            manifest_id=manifest_resource_response.resource.id,
            period_id=period_resource_response.resource.id,
            adaptationset_id=adaptationset_resource_response.resource.id,
    ...
    def _compare_content_protections(self, first: ContentProtection, second: ContentProtection):
        self.assertEqual(first.encodingId, second.encodingId)
        self.assertEqual(first.muxingId, second.muxingId)
        self.assertEqual(first.drmId, second.drmId)
        return True
    ...
    def _get_sample_muxing(self):
        output = self._get_sample_encoding_output()
        muxing_stream = MuxingStream(stream_id=self.sampleStream.id)
    ...
    def _create_sample_stream(self):
        ...
        return stream_resource_response.resource
    ...
    def _get_sample_content_protection(self):
        content_protection = ContentProtection(encoding_id=self.sampleEncoding.id,
                                               muxing_id=self.sampleMuxing.id,
                                               drm_id=self.sampleDrm.id)
