...
class RawID3TagTests(BitmovinTestCase):
    ...
    def test_create_id3_raw(self):
        ts_muxing = self._create_muxing()  # type: ProgressiveTSMuxing
        self.assertIsNotNone(ts_muxing.id)
        raw_id3_tag = self._get_sample_id3_raw_time()
        created_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.Raw.create(
            object_=raw_id3_tag,
        ...
        id3_resource = created_id3_response.resource  # type: RawID3Tag
        self._compare_raw_id3_tags(raw_id3_tag, id3_resource)

    @unittest.skip("Not implemented yet")
    def test_create_id3_raw_frame(self):
        ts_muxing = self._create_muxing()  # type: ProgressiveTSMuxing
        self.assertIsNotNone(ts_muxing.id)
        raw_id3_tag = self._get_sample_id3_raw_frame()
        created_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.Raw.create(
            object_=raw_id3_tag,
            encoding_id=self.sampleEncoding.id,
            muxing_id=ts_muxing.id
    ...
    def test_create_id3_raw_without_name(self):
        ts_muxing = self._create_muxing()  # type: ProgressiveTSMuxing
        self.assertIsNotNone(ts_muxing.id)
        raw_id3_tag = self._get_sample_id3_raw_time()
        raw_id3_tag.name = None
        created_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.Raw.create(
            object_=raw_id3_tag,
            encoding_id=self.sampleEncoding.id,
            muxing_id=ts_muxing.id
    ...
    def test_retrieve_raw_id3(self):
        ts_muxing = self._create_muxing()  # type: ProgressiveTSMuxing
        self.assertIsNotNone(ts_muxing.id)
        raw_id3_tag = self._get_sample_id3_raw_time()
        created_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.Raw.create(
        ...
        retrieved_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.Raw.retrieve(
            encoding_id=self.sampleEncoding.id,
            muxing_id=ts_muxing.id,
        ...
        self._compare_raw_id3_tags(retrieved_id3_response.resource, created_id3_response.resource)

    @unittest.skip("Not implemented yet")
    def test_retrieve_raw_id3_frame(self):
        ts_muxing = self._create_muxing()  # type: ProgressiveTSMuxing
        self.assertIsNotNone(ts_muxing.id)
        raw_id3_tag = self._get_sample_id3_raw_frame()
        created_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.Raw.create(
    ...
    def test_delete_raw_id3(self):
        ...
        deleted_minimal_resource = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.Raw.delete(
            muxing_id=ts_muxing.id,
            encoding_id=self.sampleEncoding.id,
            id3_id=id3_resource.id
        )
        ...
        try:
            self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.Raw.retrieve(encoding_id=self.sampleEncoding.id,
                                                                              muxing_id=ts_muxing.id,
                                                                              id3_id=id3_resource.id)
            self.fail(
    ...
    def test_list_raw_id3_tags(self):
        ts_muxing = self._create_muxing()  # type: ProgressiveTSMuxing
        self.assertIsNotNone(ts_muxing.id)
        raw_id3_tag = self._get_sample_id3_raw_time()
        created_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.Raw.create(
        ...
        raw_id3_tags = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.Raw.list(encoding_id=self.sampleEncoding.id,
                                                                                     muxing_id=ts_muxing.id)
        self.assertIsNotNone(raw_id3_tags)
        self.assertIsNotNone(raw_id3_tags.resource)
        self.assertIsNotNone(raw_id3_tags.response)
        self.assertIsInstance(raw_id3_tags.resource, list)
        self.assertIsInstance(raw_id3_tags.response, Response)
        self.assertGreater(raw_id3_tags.resource.__sizeof__(), 1)

    def test_retrieve_id3_custom_data(self):
        ts_muxing = self._create_muxing()  # type: ProgressiveTSMuxing
        self.assertIsNotNone(ts_muxing.id)
        raw_id3_tag = self._get_sample_id3_raw_time()
        raw_id3_tag.customData = {
            "myKey": "MY_VALUE"
        }
        created_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.Raw.create(
            object_=raw_id3_tag,
        ...
        custom_data_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.Raw.retrieve_custom_data(
            muxing_id=ts_muxing.id,
            encoding_id=self.sampleEncoding.id,
            id3_id=id3_resource.id
        )
        ...
        self.assertEqual(raw_id3_tag.customData, json.loads(custom_data.customData))
    ...
    def _compare_raw_id3_tags(self, first: RawID3Tag, second: RawID3Tag):
    ...
    def _get_sample_id3_raw_frame(self):

        id3 = RawID3Tag(position_mode=ID3TagPositionMode.FRAME, frame=1234,
                        bytes_=base64.b64encode(b'MyTag Python TeSt').decode('utf-8'),
                        name='RawID3 Frame', description='no descr')

        return id3

    def _get_sample_id3_raw_time(self):
