...
from bitmovin import Bitmovin, Response, Stream, StreamInput, EncodingOutput, ACLEntry, Encoding, \
    ProgressiveTSMuxing, MuxingStream, SelectionMode, ACLPermission, PlainTextID3Tag, ID3TagPositionMode
from bitmovin.errors import BitmovinApiError
from tests.bitmovin import BitmovinTestCase


class PlainTextID3TagTests(BitmovinTestCase):
    ...
    def test_create_id3_plain_text(self):
        ts_muxing = self._create_muxing()  # type: ProgressiveTSMuxing
        self.assertIsNotNone(ts_muxing.id)
        plain_text_id3_tag = self._get_sample_id3_plain_text_time()
        created_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.PlainText.create(
            object_=plain_text_id3_tag,
        ...
        id3_resource = created_id3_response.resource  # type: PlainTextID3Tag
        self._compare_plain_text_id3_tags(plain_text_id3_tag, id3_resource)

    @unittest.skip("Not implemented yet")
    def test_create_id3_plain_text_frame(self):
        ts_muxing = self._create_muxing()  # type: ProgressiveTSMuxing
        self.assertIsNotNone(ts_muxing.id)
        plain_text_id3_tag = self._get_sample_id3_plain_text_frame()
        created_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.PlainText.create(
            object_=plain_text_id3_tag,
            encoding_id=self.sampleEncoding.id,
            muxing_id=ts_muxing.id
    ...
    def test_create_id3_plain_text_without_name(self):
        ts_muxing = self._create_muxing()  # type: ProgressiveTSMuxing
        self.assertIsNotNone(ts_muxing.id)
        plain_text_id3_tag = self._get_sample_id3_plain_text_time()
        plain_text_id3_tag.name = None
        created_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.PlainText.create(
            object_=plain_text_id3_tag,
            encoding_id=self.sampleEncoding.id,
            muxing_id=ts_muxing.id
    ...
    def test_retrieve_plain_text_id3(self):
        ts_muxing = self._create_muxing()  # type: ProgressiveTSMuxing
        self.assertIsNotNone(ts_muxing.id)
        plain_text_id3_tag = self._get_sample_id3_plain_text_time()
        created_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.PlainText.create(
        ...
        retrieved_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.PlainText.retrieve(
            encoding_id=self.sampleEncoding.id,
            muxing_id=ts_muxing.id,
        ...
        self._compare_plain_text_id3_tags(retrieved_id3_response.resource, created_id3_response.resource)

    @unittest.skip("Not implemented yet")
    def test_retrieve_plain_text_id3_frame(self):
        ts_muxing = self._create_muxing()  # type: ProgressiveTSMuxing
        self.assertIsNotNone(ts_muxing.id)
        plain_text_id3_tag = self._get_sample_id3_plain_text_frame()
        created_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.PlainText.create(
    ...
    def test_delete_plain_text_id3(self):
        ts_muxing = self._create_muxing()  # type: ProgressiveTSMuxing
        self.assertIsNotNone(ts_muxing.id)
        plain_text_id3_tag = self._get_sample_id3_plain_text_time()
        created_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.PlainText.create(
        ...
        deleted_minimal_resource = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.PlainText.delete(
            muxing_id=ts_muxing.id,
            encoding_id=self.sampleEncoding.id,
            id3_id=id3_resource.id
        )
        ...
        try:
            self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.PlainText.retrieve(
                encoding_id=self.sampleEncoding.id,
                muxing_id=ts_muxing.id,
                id3_id=id3_resource.id
            )
    ...
    def test_list_plain_text_id3_tags(self):
        ts_muxing = self._create_muxing()  # type: ProgressiveTSMuxing
        self.assertIsNotNone(ts_muxing.id)
        plain_text_id3_tag = self._get_sample_id3_plain_text_time()
        created_id3_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.PlainText.create(
        ...
        plain_text_id3_tags = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.PlainText.list(
            encoding_id=self.sampleEncoding.id,
            muxing_id=ts_muxing.id
        )
        self.assertIsNotNone(plain_text_id3_tags)
        self.assertIsNotNone(plain_text_id3_tags.resource)
        self.assertIsNotNone(plain_text_id3_tags.response)
        self.assertIsInstance(plain_text_id3_tags.resource, list)
    ...
    def test_retrieve_id3_custom_data(self):
        ...
        custom_data_response = self.bitmovin.encodings.Muxing.ProgressiveTS.ID3Tags.PlainText.retrieve_custom_data(
            muxing_id=ts_muxing.id,
            encoding_id=self.sampleEncoding.id,
            id3_id=id3_resource.id
        )
        ...
        self.assertEqual(plain_text_id3_tag.customData, json.loads(custom_data.customData))
    ...
    def _compare_plain_text_id3_tags(self, first: PlainTextID3Tag, second: PlainTextID3Tag):
        self.assertEqual(first.text, second.text)
        self.assertEqual(first.frameId, second.frameId)
    ...
    def _get_sample_id3_plain_text_frame(self):

        id3 = PlainTextID3Tag(position_mode=ID3TagPositionMode.FRAME, frame=1234,
                              text='MyTag Python TeSt', frame_id='AVIA',
                              name='PlainTextID3 Frame', description='no descr')

        return id3

    def _get_sample_id3_plain_text_time(self):
