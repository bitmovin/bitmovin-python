...
from bitmovin import Bitmovin, Response, Stream, StreamInput, EncodingOutput, ACLEntry, Encoding, \
    ProgressiveTSMuxing, MuxingStream, ACLPermission, SelectionMode, StreamConditionsMode, InternalChunkLengthMode, \
    InternalChunkLength
...
class EncodingProgressiveTSMuxingTests(BitmovinTestCase):
    ...
    def test_create_muxing(self):
        ...
        muxing_resource_response = self.bitmovin.encodings.Muxing.ProgressiveTS.create(
    ...
    def test_retrieve_muxing(self):
        ...
        retrieved_muxing_response = self.bitmovin.encodings.Muxing.ProgressiveTS.retrieve(
    ...
    def test_delete_muxing(self):
        ...
        deleted_minimal_resource = self.bitmovin.encodings.Muxing.ProgressiveTS.delete(
            muxing_id=created_muxing_response.resource.id,
            encoding_id=self.sampleEncoding.id
        )
        ...
        try:
            self.bitmovin.encodings.Muxing.ProgressiveTS.retrieve(created_muxing_response.resource.id)
            self.fail(
    ...
    def test_list_muxings(self):
        ...
        muxings = self.bitmovin.encodings.Muxing.ProgressiveTS.list(encoding_id=self.sampleEncoding.id)
    ...
    def test_retrieve_stream_custom_data(self):
        ...
        custom_data_response = self.bitmovin.encodings.Muxing.ProgressiveTS.retrieve_custom_data(
            muxing_id=created_muxing_response.resource.id,
            encoding_id=self.sampleEncoding.id
        )
    ...
    def _get_sample_muxing(self):
        ...
        muxing = ProgressiveTSMuxing(streams=[muxing_stream], segment_length=4, filename='myprogressive.ts',
                                     outputs=stream.outputs, name='Sample ProgressiveTSMuxing')
        return muxing
