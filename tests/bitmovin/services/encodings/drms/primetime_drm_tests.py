...
from bitmovin import Bitmovin, Response, Stream, StreamInput, EncodingOutput, ACLEntry, Encoding, \
    FMP4Muxing, MuxingStream, PrimeTimeDRM, DRMStatus, SelectionMode, ACLPermission
from bitmovin.errors import BitmovinApiError
from tests.bitmovin import BitmovinTestCase


class PrimeTimeDRMTests(BitmovinTestCase):
    ...
    def test_create_drm(self):
        ...
        sample_drm = self._get_sample_drm_primetime()
        sample_drm.outputs = fmp4_muxing.outputs

        created_drm_response = self.bitmovin.encodings.Muxing.FMP4.DRM.PrimeTime.create(
        ...
        drm_resource = created_drm_response.resource  # type: PrimeTimeDRM
    ...
    def test_retrieve_drm(self):
        ...
        retrieved_drm_response = self.bitmovin.encodings.Muxing.FMP4.DRM.PrimeTime.retrieve(
    ...
    def test_delete_drm(self):
        ...
        deleted_minimal_resource = self.bitmovin.encodings.Muxing.FMP4.DRM.PrimeTime.delete(
        ...
        try:
            self.bitmovin.encodings.Muxing.FMP4.DRM.PrimeTime.retrieve(encoding_id=self.sampleEncoding.id,
    ...
    def test_list_drms(self):
        ...
        drms = self.bitmovin.encodings.Muxing.FMP4.DRM.PrimeTime.list(encoding_id=self.sampleEncoding.id,
    ...
    def test_retrieve_stream_custom_data(self):
        ...
        custom_data_response = self.bitmovin.encodings.Muxing.FMP4.DRM.PrimeTime.retrieve_custom_data(
            muxing_id=fmp4_muxing.id,
    ...
    def test_retrieve_drm_status(self):
        ...
        drm_status_response = self.bitmovin.encodings.Muxing.FMP4.DRM.PrimeTime.retrieve_status(
            muxing_id=fmp4_muxing.id,
            encoding_id=self.sampleEncoding.id,
            drm_id=drm_resource.id
        )
    ...
    def _compare_drms(self, first: PrimeTimeDRM, second: PrimeTimeDRM):
        """
    ...
    def _get_sample_drm_primetime(self):
        primetime_drm_settings = self.settings.get('sampleObjects').get('drmConfigurations').get('PrimeTime')

        drm = PrimeTimeDRM(key=primetime_drm_settings[0].get('key'),
                           kid=primetime_drm_settings[0].get('kid'),
                           pssh=primetime_drm_settings[0].get('pssh'),
                           name='Sample PrimeTime DRM')
