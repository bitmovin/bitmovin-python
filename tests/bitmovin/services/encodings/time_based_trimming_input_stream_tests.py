import unittest
from bitmovin import Bitmovin, Response, IngestInputStream, TimeBasedTrimmingInputStream, Encoding, SelectionMode
from bitmovin.errors import BitmovinApiError
from tests.bitmovin import BitmovinTestCase


class EncodingTimeBasedTrimmingInputStreamTests(BitmovinTestCase):
    ...
    def test_create_time_based_trimming_input_stream(self):
        sample_time_based_trimming_input_stream = self._get_sample_time_based_trimming_input_stream(
            input_stream_id=self.sampleIngestInputStream.id
        )

        trimming_input_stream_resource_response = self.bitmovin.encodings.TimeBasedTrimmingInputStream.create(
            object_=sample_time_based_trimming_input_stream,
        ...
        self._compare_time_based_trimming_input_streams(
            first=sample_time_based_trimming_input_stream,
            second=trimming_input_stream_resource_response.resource
        )

    def test_retrieve_time_based_trimming_input_stream(self):
        sample_time_based_trimming_input_stream = self._get_sample_time_based_trimming_input_stream(
            input_stream_id=self.sampleIngestInputStream.id
        )
        ...
        retrieved_trimming_input_stream_response = self.bitmovin.encodings.TimeBasedTrimmingInputStream.retrieve(
            encoding_id=self.sampleEncoding.id,
    ...
    def test_delete_time_based_trimming_input_stream(self):
        sample_time_based_trimming_input_stream = self._get_sample_time_based_trimming_input_stream(
            input_stream_id=self.sampleIngestInputStream.id
        )
        ...
        deleted_minimal_resource = self.bitmovin.encodings.TimeBasedTrimmingInputStream.delete(
            encoding_id=self.sampleEncoding.id,
            input_stream_id=trimming_input_stream_resource_response.resource.id
        )
        ...
        with self.assertRaises(BitmovinApiError):
            self.bitmovin.encodings.TimeBasedTrimmingInputStream.retrieve(
                encoding_id=self.sampleEncoding.id,
                input_stream_id=trimming_input_stream_resource_response.resource.id
            )

    def test_list_time_based_trimming_input_streams(self):
        sample_time_based_trimming_input_stream = self._get_sample_time_based_trimming_input_stream(
            input_stream_id=self.sampleIngestInputStream.id
        )
        ...
        trimming_input_streams = self.bitmovin.encodings.TimeBasedTrimmingInputStream.list(
            encoding_id=self.sampleEncoding.id
        )
        ...
        self.assertIsInstance(retrieved_input_stream, TimeBasedTrimmingInputStream)
        self._compare_time_based_trimming_input_streams(
    ...
    def _compare_time_based_trimming_input_streams(self,
                                                   first: TimeBasedTrimmingInputStream,
                                                   second: TimeBasedTrimmingInputStream):
        """

        :param first: TimeBasedTrimmingInputStream
        :param second: TimeBasedTrimmingInputStream
        :return: bool
        """
        self.assertEqual(first.name, second.name)
        self.assertEqual(first.description, second.description)
    ...
    def _get_sample_time_based_trimming_input_stream(self, input_stream_id):
        input_stream = TimeBasedTrimmingInputStream(input_stream_id=input_stream_id,
                                                    offset=5.22,
                                                    duration=128.2,
                                                    name='Sample TimeBased Trimming Input Stream',
                                                    description='TBTIS Python IT')
