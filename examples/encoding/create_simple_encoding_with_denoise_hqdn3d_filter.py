...
from bitmovin import Bitmovin, Encoding, S3Output, H264CodecConfiguration, StreamFilter, DenoiseHqdn3dFilter, \
...
def main():
    ...
    encoding = Encoding(name='Encoding with denoise hqdn3d filter',
    ...
    # create the denoise hqdn3d filter.
    denoise_filter = DenoiseHqdn3dFilter(name="Denoise Hqdn3d Filter",
                                         luma_spatial=3.0,
                                         chroma_spatial=4.0,
                                         luma_tmp=6.0,
                                         chroma_tmp=4.5)
    denoise_filter = bitmovin.filters.DenoiseHqdn3d.create(object_=denoise_filter).resource

    stream_filter = [StreamFilter(id=denoise_filter.id, position=0)]
