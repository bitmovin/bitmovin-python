...
from bitmovin import Bitmovin, Encoding, HTTPSInput, S3Output, H264CodecConfiguration, \
    ...
    VideoAdaptationSet, AudioAdaptationSet, IngestInputStream, ConcatenationInputStreamConfiguration, \
...
def main():
    ...
    bumper_https_input = HTTPSInput(name='create_simple_encoding HTTPS input', host=BUMPER_HTTPS_INPUT_HOST)
    bumper_https_input = bitmovin.inputs.HTTPS.create(bumper_https_input).resource

    main_https_input = HTTPSInput(name='create_simple_encoding HTTPS input', host=MAIN_HTTPS_INPUT_HOST)
    ...
    bumper_ingest_input_stream = IngestInputStream(input_id=bumper_https_input.id,
    ...
    bumper_concatenation_input_stream_config = ConcatenationInputStreamConfiguration(input_stream_id=bumper_ingest_input_stream.id,
                                                                                     is_main=False,
                                                                                     position=0)

    main_concatenation_input_stream_config = ConcatenationInputStreamConfiguration(input_stream_id=main_ingest_input_stream.id,
                                                                                     is_main=True,
                                                                                     position=0)
    ...
    video_stream_720p = Stream(codec_configuration_id=video_codec_configuration_720p.id,
                               input_streams=[input_stream], name='Sample Stream 720p')
    ...
    audio_stream = Stream(codec_configuration_id=audio_codec_configuration.id,
                          input_streams=[input_stream], name='Sample Stream AUDIO')
