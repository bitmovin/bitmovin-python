...
from bitmovin import Bitmovin, Encoding, S3Output, H264CodecConfiguration, \
    ...
    FMP4Representation, FMP4RepresentationType, LiveStreamConfiguration, LiveDashManifest, GenericS3Output, TSMuxing, LiveHlsManifest, HlsManifest, AudioMedia, VariantStream
from bitmovin.errors import BitmovinApiError

ERROR_CODE_LIVE_STREAM_NOT_READY = 2023
LIVE_STREAM_INFORMATION_FETCH_RETRY_INTERVAL = 5
LIVE_STREAM_INFORMATION_FETCH_MAX_RETRIES = 100
...
GENERIC_S3_OUTPUT_ACCESSKEY = '<YOUR_GENERIC_S3_ACCESS_KEY>'
GENERIC_S3_OUTPUT_SECRETKEY = '<YOUR_GENERIC_S3_OUTPUT_SECRETKEY>'
GENERIC_S3_OUTPUT_BUCKETNAME = '<YOUR_GENERIC_S3_OUTPUT_BUCKETNAME>'
GENERIC_S3_OUTPUT_HOST = '<YOUR_GENERIC_S3_HOST>'
GENERIC_S3_OUTPUT_PORT = 8080 # YOUR_GENERIC_S3_PORT
...
def main():
    ...
    manifest_output = EncodingOutput(output_id=output.id,
                                     ...
                                     acl=[acl_entry])
    hls_manifest = HlsManifest(manifest_name='example_live_manifest.m3u8',
...
def print_live_stream_details(encoding_id, live_stream_details):
    rtmp_service="enc-{}-bitcodin-rtmp".format(encoding_id)
    node_port_patch_command="kubectl patch service {} -p '{{ \"spec\": {{ \"type\": \"NodePort\" }} }}'".format(rtmp_service)
    query_node_port="kubectl get service {} -o json | jq '.spec.ports[] | select(.name==\"rtmp-ingest-point\").nodePort'".format(rtmp_service)
    node_port_patch_custom_id="kubectl patch service {} -p '{{ \"spec\": {{ \"type\": \"NodePort\" \"nodePort\": \"30001\" }} }}'".format(rtmp_service)
    load_balancer="kubectl patch service {} -p '{{ \"spec\": {{ \"type\": \"LoadBalancer\" }} }}'".format(rtmp_service)
    load_balancer_ip_and_port="kubectl get service {} -o json | jq -r '.status.loadBalancer.ingress[] | .ip'".format(rtmp_service)
    ...
    print('')
    print('Stream URL: ... {}'.format('rtmp://<public-ip>:<port>/live'.format(live_stream_details.encoderIp)))
    print('-----------------------------------------------------')
    print('You can now configure the RTMP ingest point for access outside of your infrastructure.')
    print('Configure node port access ............................ {}'.format(node_port_patch_command))
    print('Configure node port access (custom port) .............. {}'.format(node_port_patch_custom_id))
    print('Find the node port .................................... {}'.format(query_node_port))
    print('Configure public load balancer ........................ {}'.format(load_balancer))
    print('Find out the ip of the load balancer (use port 1935)... {}'.format(load_balancer_ip_and_port))
    print('\n')
